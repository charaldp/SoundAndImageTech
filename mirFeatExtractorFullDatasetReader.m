...
%audio = miraudio('E:\Downloads\Media\MusicSpeechDetection\music_speech\music_speech\music_wav\bagpipe.wav');
%audio = miraudio('E:\Downloads\Media\MusicSpeechDetection\music_speech\music_speech\custom\full-mus-spe3.wav');
%mirplay(audio);
GTZAN_MusicFileList = dir('E:\Downloads\Media\MusicSpeechDetection\music_speech\music_speech\music_wav\*.wav');
GTZAN_SpeechFileList = dir('E:\Downloads\Media\MusicSpeechDetection\music_speech\music_speech\speech_wav\*.wav');

Musan_MusicFileList = dir('E:\Downloads\Media\MusicSpeechDetection\musan\music\*\*.wav');
Musan_SpeechFileList = dir('E:\Downloads\Media\MusicSpeechDetection\musan\speech\*\*.wav');

MIREX_FileList = dir('E:\Downloads\Media\MusicSpeechDetection\muspeak-mirex2015-detection-examples\*.mp3');
MIREX_CSVList = dir('E:\Downloads\Media\MusicSpeechDetection\muspeak-mirex2015-detection-examples\*.csv');
TimestampData = csvread();
datasetsInfo={GTZAN_MusicFileList,GTZAN_SpeechFileList,Musan_MusicFileList,Musan_SpeechFileList};
for i = 1:length(datasetsInfo)
    class = mod(i,2);
    for j = 1:length(datasetsInfo{j})
        audio = miraudio(sprintf('%s%s',datasetsInfo{j}.folder,datasetsInfo{j}.name));
        framedAudio = mirframe(audio,'Length',0.5,'s');
...
for i = 1:length(raw_spectral_irregularity)
    raw_class(i) = 1 + round(mod(i,length(raw_spectral_irregularity)/3)/length(raw_spectral_irregularity)*3);
end
raw_class(length(raw_spectral_irregularity)) = 2;
...
data = normalized_data(~any(isnan(normalized_data')),:);
sizeFin = size(data,1);
...
tic;
[rank,w] = relieff(data(:,1:M1-1),data(:,M1),20);
...
for k = 1:30000
    idx = randperm(N1);
    % Shuffle data
    audio_data = data(idx,1:M1);
    % Create Cross Validation Partitions
    CVO = cvpartition(audio_data(:,M1),'KFold',5);
    for l=1:2
        for m = 1:5
            partition_indexes(m,l) = mean(audio_data(CVO.test(m),M1)==l-1);
        end
    end
...
for i=1:length(NF)
    ...
    for j=1:length(NR)
        ...
        TSK_model = genfis2(audio_data(:,features),audio_data(:,M1),Optimal_radius(i,j) ,xBounds(:,[features M1]) ,sbcOptions);
        ...
        parfor k = 1:CVO.NumTestSets
            ...
            [TSK_Model_HD(k),t_error,~,TSK_Model_CLSS,c_error] = ...
                anfis(audio_data(CVO.training(k),[features M1]), TSK_Model_HD(k), t_opt, d_opt, audio_data(CVO.test(k),[features M1]));
...
while flag
    for i = 1:30000
        idx = randperm(N1);
        D_trn = data(idx(1:ceil(0.6*N1)),features);
        flag = false;
        ...
        if ~flag
            D_val = data(idx(ceil(0.6*N1)+1:ceil(0.8*N1)),features);
            D_chk = data(idx(ceil(0.8*N1)+1:N1),features);
            for k=1:clsNum
...
TSK_Model_Final = genfis2(data(:,features(1:optNF)),data(:,M1), optRadius ,xBounds(:,features) ,sbcOptions);
